...
struct covariance_regularization_normalize_mineig {
    __host__ __device__ void operator()(Eigen::Matrix3f& cov) const {
        Eigen::SelfAdjointEigenSolver<Eigen::Matrix3f> eig;
        eig.computeDirect(cov);

        Eigen::Vector3f values = eig.eigenvalues();
        float max_value = values[2];
//        for (int i = 0; i < 3; i++) {
//            max_value = fmaxf(max_value, values[i]);
//        }
        for (int i = 0; i < 3; i++) {
            values[i] = fmaxf(1e-3f, values[i]/max_value);
        }
...
void covariance_regularization(thrust::device_vector<Eigen::Vector3f>& means, thrust::device_vector<Eigen::Matrix3f>& covs, RegularizationMethod method) {
  ...
  } else if (method == RegularizationMethod::NORMALIZED_MIN_EIG) {
      thrust::for_each(covs.begin(), covs.end(), covariance_regularization_normalize_mineig());
  }
  else {
